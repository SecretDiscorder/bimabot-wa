// Tangkap error global supaya bot tidak mati
...
async function reconnectWithBackoff(attempt = 1) {
    const delay = Math.min(30000, 1000 * 2 ** attempt); // max 30s
    ...
    try {
        ...
        sock.ev.on('messages.upsert', async ({
			...
			try {
				...
				// pencegahan rate limit
				if (cmd === '!factorial' && parseInt(args[1]) > 170) {
					return sock.sendMessage(msg.key.remoteJid, { text: 'Angka terlalu besar!' });
				}
				if ((cmd === '!virtex' || cmd === '!spam' || cmd === '!atur') && parseInt(args[2]) > 10) {
					return sock.sendMessage(msg.key.remoteJid, { text: 'Batas maksimal 10!' });
				}
				...
				const ayatFile = './ayat.json';
				if (!fs.existsSync(ayatFile)) {
					const allAyats = quranAyats.getAllAyats();
					fs.writeFileSync(ayatFile, JSON.stringify(allAyats));
				}
